...
public class CommerceInventoryWarehouseItemLocalServiceImpl
		...
		List<Object[]> sumStocks = dslQuery(
			DSLQueryFactoryUtil.select(
				...
				DSLFunctionFactoryUtil.min(
					BookedQuantityTable.INSTANCE.sumBooked
				).as(
					BookedQuantityTable.INSTANCE.sumBooked.getName()
				),
				DSLFunctionFactoryUtil.min(
					ReplenishmentQuantityTable.INSTANCE.sumAwaiting
				).as(
					ReplenishmentQuantityTable.INSTANCE.sumAwaiting.getName()
				)
			).from(
				CommerceInventoryWarehouseItemTable.INSTANCE
			).leftJoinOn(
				...
				CommerceInventoryWarehouseItemTable.INSTANCE.sku.eq(
					BookedQuantityTable.INSTANCE.sku)
			).leftJoinOn(
				ReplenishmentQuantityTable.INSTANCE.getQueryTable(companyId),
				CommerceInventoryWarehouseItemTable.INSTANCE.sku.eq(
					ReplenishmentQuantityTable.INSTANCE.sku)
			).where(
				CommerceInventoryWarehouseItemTable.INSTANCE.companyId.eq(
					companyId
				).and(
	...
	public void moveQuantitiesBetweenWarehouses(
		...
		if (quantity.compareTo(fromWarehouseItem.getQuantity()) == 1) {
	...
	private static class BookedQuantityTable
		...
		public QueryTable getQueryTable(long companyId) {
			return new QueryTable(
				...
				Arrays.asList(sku, sumBooked));
		}

		public final Column<BookedQuantityTable, String> sku = createColumn(
			"SKU", String.class, Types.VARCHAR, Column.FLAG_DEFAULT);
		public final Column<BookedQuantityTable, Integer> sumBooked =
			createColumn(
	...
	private static class ReplenishmentQuantityTable
		...
		public QueryTable getQueryTable(long companyId) {
			return new QueryTable(
				...
				Arrays.asList(sku, sumAwaiting));
		}

		public final Column<ReplenishmentQuantityTable, String> sku =
			createColumn(
				"SKU", String.class, Types.VARCHAR, Column.FLAG_DEFAULT);
		public final Column<ReplenishmentQuantityTable, Integer> sumAwaiting =
			createColumn(
