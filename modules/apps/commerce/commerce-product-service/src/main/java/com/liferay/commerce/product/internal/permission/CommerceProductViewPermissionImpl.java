...
public class CommerceProductViewPermissionImpl
	...
	public boolean contains(
		...
		if(_isProductExcluded(commerceAccountId, cpDefinition)){
			return false;
		}
	...
	private boolean _isProductExcluded(long commerceAccountId, CPDefinition cpDefinition) throws PortalException{

		long cpProductId = cpDefinition.getCProductId();

		List<Long> accountExcludedProductsIds =
			_accountEntryHelper.getAccountExcludedProductsIds(commerceAccountId);
	...
	private boolean _hasMatchingCategory(List<Long> ids, List<AssetCategory> categories) {
