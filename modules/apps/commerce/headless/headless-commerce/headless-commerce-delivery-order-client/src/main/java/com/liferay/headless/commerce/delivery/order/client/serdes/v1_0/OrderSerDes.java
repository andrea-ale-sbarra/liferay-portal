...
public class OrderSerDes {
	...
	public static String toJSON(Order order) {
		...
		if (order.getOrderComments() != null) {
			...
			for (int i = 0; i < order.getOrderComments().length; i++) {
				sb.append(String.valueOf(order.getOrderComments()[i]));

				if ((i + 1) < order.getOrderComments().length) {
					sb.append(", ");
				}
			}
	...
	public static Map<String, String> toMap(Order order) {
		...
		if (order.getOrderComments() == null) {
			map.put("orderComments", null);
		}
		else {
			map.put("orderComments", String.valueOf(order.getOrderComments()));
		}
	...
	public static class OrderJSONParser extends BaseJSONParser<Order> {
		...
		protected void setField(
			...
			else if (Objects.equals(jsonParserFieldName, "orderComments")) {
				if (jsonParserFieldValue != null) {
