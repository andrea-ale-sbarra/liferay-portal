...
public class ObjectEntryModelListener extends BaseModelListener<ObjectEntry> {
	...
	public void onBeforeCreate(ObjectEntry objectEntry)
		...
		try {
			if (!_isPIMProductObjectEntry(objectEntry) ||
				!_isDefaultContentPlacement(objectEntry)) {
	...
	private boolean _isDefaultContentPlacement(ObjectEntry objectEntry) {

		// A depot-scoped CMS object entry created without an explicit folder is
		// placed in L_CONTENTS by the REST/batch manager (see
		// DefaultObjectEntryManagerImpl#_getObjectEntryFolderId), while a
		// programmatic create can leave it at the default parent. Both mean "no
		// folder was chosen", so the product is routed to L_PRODUCTS. An
		// explicit folder (L_PRODUCTS or any subfolder) is left untouched.
